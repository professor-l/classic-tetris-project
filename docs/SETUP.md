...
NOTE: If you wish to use another database in place of SQLite for your development environment, you are expected to understand the setup process for that on your own. This document assumes use of SQLite.
...
Alternatively, if you want to install it with Python 3 (which I did):
...
Using your preferred text editor (either a command-line one like `nano` or `vim`, or a GUI one if you are more comfortable there), edit the `requirements.txt` file to add a `#` in front of the line that starts with `mysqlclient==` (it's line 9 right now, but that is subject to change). Save the file, then, from within the projects root directory (`classic-tetris-project`), run the following command to install the requirements:

```bash
pip install -r requirements.txt
```

Now, before you forget, **immediately** remove the comment from the `requirements.txt` file, reverting it back to its original state. Alternatively, `git reset --hard` to reset all files if for some reason you've edited other files.

We're almost done! We just have two more steps. Right now, there's still no database of any kind. Thankfullly, with SQLite, there's no setup involved, because Django will automatically create a database upon the first migration if one doesn't exist already. Still within the project's root directory, run this:
