...
class TournamentSheetUpdater:
    ...
    def match_data(self, match):
        ...
        return {
            ...
            PLAYER_1_SEED: match.player1.seed if match.player1 else None,
            PLAYER_1_WINS: match.match.wins1 if match.match and match.match.ended_at else None,
            PLAYER_2: match_display.player2_display_name(),
            PLAYER_2_SEED: match.player2.seed if match.player2 else None,
            PLAYER_2_WINS: match.match.wins2 if match.match and match.match.ended_at else None,
            WINNER: match.winner.display_name() if match.winner else None,
            LOSER: match.loser.display_name() if match.loser else None,
            START_TIME: match.match.start_date.isoformat() if match.match and match.match.start_date else None,
            CHANNEL: match.match.channel.name if match.match and match.match.channel else None,
            VOD: match.match.vod if match.match else None,
