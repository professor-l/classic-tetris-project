...
class HzSimulation(object):
    ...
    def generate_sequence(self, num_taps):       
        if self.level < 0 or self.height < 0 or self.height > 19 or self.taps < 1 or self.taps > 5:
            raise ValueError("`Unrealistic parameters.`")

        if self.taps == 1:
            raise ValueError("`You have {fr} frames to time this tap (and maybe a rotation for polevault).`".format(
                fr = self.frames
            ))
